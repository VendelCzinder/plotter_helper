...
namespace PlotterHelper {
    ...
    static class Logic {
        ...
        private const int COLOR_OFFSET = 200; // in pixel intensity [0-255]
        ...
        private static void DrawLines(Bitmap bmp, Graphics graphics, int totalWidth, float lineLength, float lineTop, 
            ...
            Color offsetColor = GetOffsetColor4(averageColor, COLOR_OFFSET);
            ...
            averageColor = GetAverageColor(bmp, (int)x1,
                (int)(lineTop - (lineWidth / 2)),
                (int)lineLength, (int)(lineTop + (lineWidth / 2)));
            offsetColor = GetOffsetColor4(averageColor, COLOR_OFFSET);
        ...
        private static Color GetOffsetColor(Color color, int offset) {
            // variables
            int r = 0, g = 0, b = 0;
            // calculating offsets
            if (color.R > 127) {  r = color.R - offset;
            } else { r = color.R + offset; }
            if (color.G > 127) {  g = color.G - offset; }
            else { g = color.G + offset; }
            if (color.B > 127) { b = color.B - offset; }
            else { b = color.B + offset; }
            // creating color, returning
            return Color.FromArgb(r, g, b);
        }
        private static Color GetOffsetColor2(Color color, int alpha) {
        ...
        private static Color GetOffsetColor3(Color color, float offset) {
            // getting HLS
        ...
        private static Color GetOffsetColor4(Color color, int alpha) {
            // getting HLS
            (double h, double l, double s) = RgbToHls(color.R, color.G, color.B);
            // changing the luminance
            if (l > 0.5) {
